...
Game::Game() : window(sf::VideoMode(WINDOW_WIDTH, WINDOW_HEIGHT), "2048 Game"),
               ...
               score(0),
               gameOver(false),
               gameWon(false) { // 将动画持续时间从 0.5 秒增加到 1.0 秒
        ...
        for (int x = 0; x < gridSize; ++x) {
            if (grid[y][x] != 0) {
                ...
                if (currentVersion == GameVersion::MODIFIED) {
                    basePosition = sf::Vector2f(
                        GRID_OFFSET_X + x * (TILE_SIZE + TILE_MARGIN) + TILE_MARGIN,
                        GRID_OFFSET_Y + y * (TILE_SIZE + TILE_MARGIN) + TILE_MARGIN
                    );
                } else {
                    basePosition = sf::Vector2f(
                ...
                // 计算最终位置
                sf::Vector2f finalPosition = basePosition + sf::Vector2f(TILE_SIZE / 2, TILE_SIZE / 2);
                tile.setPosition(finalPosition);
                ...
                // 定义向右的偏移量
                float offsetX = 5; // 你可以根据需要调整这个值

                // 设置数字的位置为方块的左上角，并向右平移
                valueText.setPosition(basePosition.x + offsetX, basePosition.y);
